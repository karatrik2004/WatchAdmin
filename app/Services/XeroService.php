...
class XeroService
{
    ...
    private function mergeLatestCurrencyRowRaw(string $accessToken, string $base, string $code, array &$rates): void
    {
        ...
        if ($normalized === null) {
            \Log::warning('Xero GET /Currencies per-code row not usable for live FX', [
                'code' => $code,
                'currency_rate_raw' => $currencyRateRaw,
                'reason' => $currencyRateRaw <= 0 ? 'non_positive_raw'
                    : (abs($currencyRateRaw - 1.0) < 0.000001 ? 'raw_is_placeholder_one' : 'normalized_rejected'),
            ]);
    ...
    private function tryGetInvoiceRate(
        ...
        if ($resolutionTrace === 'getExchangeRate') {
            $this->logFxGetExchangeRateStep('6a_invoice_fallback_getXeroConversionRate_call', [
                'from' => $from,
                'to' => $to,
                'invoice_cutoff_date_utc' => $date !== null ? $date->format('Y-m-d') : null,
            ]);
        }
        try {
            ...
            $rounded = $this->roundRateToMatchXeroAudUsdTable($base, $from, $to, (float) $conv['rate']);
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('6b_invoice_fallback_round_applied', [
                    'rate_before_round' => (float) $conv['rate'],
                    'rate_after_round' => $rounded,
                ]);
            }
            ...
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('6b_invoice_fallback_getXeroConversionRate_exception', $payload);
            }
    ...
    private function tryGetLiveCurrenciesRate(string $from, string $to, string $base, ?string $resolutionTrace = null): ?float
    {
        ...
        if (array_key_exists($cacheKey, $this->liveCurrenciesRateCache)) {
            $cached = $this->liveCurrenciesRateCache[$cacheKey];
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('4a_live_currencies_cache_hit', [
                    'cache_key' => $cacheKey,
                    'cached_rate_or_null' => $cached,
                ]);
            }
        ...
        if ($resolutionTrace === 'getExchangeRate') {
            $this->logFxGetExchangeRateStep('4a_live_currencies_cache_miss_fetching', [
                'cache_key' => $cacheKey,
                'from' => $from,
                'to' => $to,
                'base' => $base,
        ...
        try {
            ...
            if (!$accessToken || trim($this->tenantId) === '') {
                ...
                if ($resolutionTrace === 'getExchangeRate') {
                    $this->logFxGetExchangeRateStep('4b_live_currencies_aborted_no_token_or_tenant', [
            ...
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('4b_live_currencies_http_get_bulk', ['url' => $url]);
            }
            ...
            if ($resp->ok()) {
                ...
                foreach ($allRows as $currency) {
                    ...
                    if ($normalized !== null) {
                        $rates[$code] = $normalized;
                    } elseif ($resolutionTrace === 'getExchangeRate' && ($code === $from || $code === $to)) {
                        $reason = $currencyRateRaw <= 0 ? 'non_positive_raw'
                            : (abs($currencyRateRaw - 1.0) < 0.000001 ? 'raw_is_placeholder_one' : 'normalized_rejected');
                        \Log::warning('Xero GET /Currencies row not usable for live FX', [
                            'code' => $code,
                            'currency_rate_raw' => $currencyRateRaw,
                            'reason' => $reason,
                            'hint' => 'Xero often omits real CurrencyRate on this endpoint; invoice CurrencyRate may still be populated.',
                        ]);
                    }
                }
            } else {
                ...
                if ($resolutionTrace === 'getExchangeRate') {
                    $this->logFxGetExchangeRateStep('4c_live_currencies_bulk_http_not_ok', [
                        'status' => $resp->status(),
                    ]);
                }
            }
            ...
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('4c_live_currencies_bulk_rows_parsed', [
                    'bulk_row_count' => count($allRows),
                    'rates_after_bulk_loop_keys' => array_keys($rates),
                ]);
            }
            ...
            foreach (array_unique([$from, $to]) as $leg) {
                if ($resolutionTrace === 'getExchangeRate') {
                    $this->logFxGetExchangeRateStep('4c_merge_per_code_http_before', ['leg' => $leg]);
                }
                $this->mergeLatestCurrencyRowRaw($accessToken, $base, $leg, $rates);
                if ($resolutionTrace === 'getExchangeRate') {
                    $this->logFxGetExchangeRateStep('4c_merge_per_code_http_after', [
                        'leg' => $leg,
                        'base_per_unit_leg' => $rates[$leg] ?? null,
                    ]);
                }
            }
            ...
            foreach (array_unique([$from, $to]) as $leg) {
                ...
                if ($resolutionTrace === 'getExchangeRate') {
                    $this->logFxGetExchangeRateStep('4c_fill_missing_leg_from_bulk', ['leg' => $leg]);
                }
                ...
                if ($resolutionTrace === 'getExchangeRate') {
                    $this->logFxGetExchangeRateStep('4c_fill_missing_leg_result', [
                        'leg' => $leg,
                        'filled_base_per_unit' => $rates[$leg] ?? null,
                    ]);
                }
            }
            ...
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('4d_live_currencies_rates_after_merge', [
                    'has_from' => isset($rates[$from]),
                    'has_to' => isset($rates[$to]),
                    'base_per_unit_from' => $rates[$from] ?? null,
                    'base_per_unit_to' => $rates[$to] ?? null,
            ...
            if (isset($rates[$from], $rates[$to]) && $rates[$from] > 0 && $rates[$to] > 0) {
                ...
                if ($resolutionTrace === 'getExchangeRate') {
                    $this->logFxGetExchangeRateStep('4e_live_currencies_pair_computed', [
                        'formula' => 'base_per_unit(from) / base_per_unit(to)',
                        'base_per_unit_from' => $rates[$from],
                        'base_per_unit_to' => $rates[$to],
                        'quotient' => $rate,
                    ]);
                }
            ...
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('4e_live_currencies_pair_unusable', [
                    'has_from' => isset($rates[$from]),
                    'has_to' => isset($rates[$to]),
                    'bulk_row_count' => count($allRows),
                ]);
            ...
            if ($resolutionTrace === 'getExchangeRate') {
                $this->logFxGetExchangeRateStep('4z_live_currencies_exception', ['message' => $ex->getMessage()]);
            }
        }

        if ($resolutionTrace === 'getExchangeRate') {
            $this->logFxGetExchangeRateStep('4z_live_currencies_returning_null', []);
        }

        return null;
    }

    /** Structured step logs for {@see getExchangeRate}; grep log line `FX getExchangeRate`. */
    private function logFxGetExchangeRateStep(string $step, array $context = []): void
    {
        if (!filter_var(env('XERO_FX_LOG_GET_EXCHANGE_RATE_STEPS', true), FILTER_VALIDATE_BOOLEAN)) {
            return;
        }

        \Log::info('FX getExchangeRate', array_merge([
            'step' => $step,
        ], $context));
    }
     ...
     * @throws \Exception
     *
     * Step-wise INFO logs: search logs for message "FX getExchangeRate" and context key "step".
     * Disable via env XERO_FX_LOG_GET_EXCHANGE_RATE_STEPS=false.
     */
    public function getExchangeRate(string $from, string $to, ?\DateTime $date = null): float
    {
        ...
        $this->logFxGetExchangeRateStep('1_start', [
            'input_from' => $inputFrom,
            'input_to' => $inputTo,
            'normalized_from' => $from,
            'normalized_to' => $to,
            'effective_date_utc' => $effectiveDate->format('Y-m-d'),
            'date_argument_was_null' => $date === null,
        ]);

        if ($from === $to) {
            $this->logFxGetExchangeRateStep('2_same_currency_early_exit', ['rate' => 1.0]);

            return 1.0;
        }

        if (trim($this->tenantId) === '') {
            $this->logFxGetExchangeRateStep('2_tenant_missing_abort', []);
            throw new \Exception(
                'Xero tenant ID is missing. Set XERO_TENANT_ID in .env, then run php artisan config:clear if you use config caching.'
            );
        }

        $this->logFxGetExchangeRateStep('2_tenant_ok', []);
        ...
        $preferInvoices = filter_var(env('XERO_FX_PREFER_INVOICES', true), FILTER_VALIDATE_BOOLEAN);

        $resolutionOrder = (!$isBaseLegPair || !$allowInvoiceFallback)
            ? 'live_currencies_only'
            : ($preferInvoices ? 'invoice_then_live_currencies' : 'live_currencies_then_invoice_fallback');

        $this->logFxGetExchangeRateStep('3_org_and_flags', [
            'organisation_base' => $base,
            'is_base_leg_pair' => $isBaseLegPair,
            'allow_invoice_fallback' => $allowInvoiceFallback,
            'prefer_invoices_first' => $preferInvoices,
            'resolution_order' => $resolutionOrder,
        ]);

        if ($preferInvoices && $isBaseLegPair && $allowInvoiceFallback) {
            $this->logFxGetExchangeRateStep('4_try_invoice_first_start', [
                'effective_date_utc' => $effectiveDate->format('Y-m-d'),
            ]);
            $invoiceFirstRate = $this->tryGetInvoiceRate(
                $from,
                $to,
                $base,
                $effectiveDate,
                'Exchange rate from Xero invoice',
                'Xero invoice rate unavailable (invoice-first path)',
                'getExchangeRate'
            );
            if ($invoiceFirstRate !== null) {
                $this->logFxGetExchangeRateStep('5_invoice_first_success', ['rate_returned' => $invoiceFirstRate]);
                \Log::info('Exchange rate resolved', [
                    'from' => $from,
                    'to' => $to,
                    'base' => $base,
                    'rate' => $invoiceFirstRate,
                    'source' => 'xero_invoice',
                ]);

                return $invoiceFirstRate;
            }
            $this->logFxGetExchangeRateStep('5_invoice_first_miss', [
                'detail' => 'proceeding_to_live_currencies',
            ]);
        }

        $this->logFxGetExchangeRateStep('6_try_live_currencies_start', []);
        $liveRate = $this->tryGetLiveCurrenciesRate($from, $to, $base, 'getExchangeRate'); // trace enables nested FX logs
        if ($liveRate !== null) {
            $finalRate = $this->roundRateToMatchXeroAudUsdTable($base, $from, $to, $liveRate);
            $this->logFxGetExchangeRateStep('5_live_currencies_success', [
                'live_rate_before_round' => $liveRate,
                'rate_after_round' => $finalRate,
            ]);
            \Log::info('Exchange rate resolved', [
                'from' => $from,
                'to' => $to,
        ...
        $this->logFxGetExchangeRateStep('5_live_currencies_failed', [
            'detail' => 'tryGetLiveCurrenciesRate_returned_null_see_warnings_above',
        ]);
        ...
        if (!$preferInvoices && $isBaseLegPair && $allowInvoiceFallback) {
            ...
            $this->logFxGetExchangeRateStep('7_invoice_after_live_fallback_start', [
                'effective_date_utc' => $effectiveDate->format('Y-m-d'),
            ]);
            ...
            if ($rate !== null) {
                $this->logFxGetExchangeRateStep('7_invoice_fallback_success', [
                    'rate_returned' => $rate,
                ]);
                \Log::info('Exchange rate resolved', [
                    'from' => $from,
            ...
            $this->logFxGetExchangeRateStep('7_invoice_fallback_failed', [
                'detail' => 'tryGetInvoiceRate_returned_null_check_debug_logs',
            ]);
        } else {
            $this->logFxGetExchangeRateStep('6_invoice_fallback_skipped', [
                'is_base_leg_pair' => $isBaseLegPair,
                'allow_invoice_fallback' => $allowInvoiceFallback,
                'reason' => !$isBaseLegPair
                    ? 'foreign_to_foreign_requires_live_currencies_for_both_legs'
                    : 'invoice_fallback_disabled_by_env',
            ]);
        }

        $this->logFxGetExchangeRateStep('8_abort_no_usable_rate', [
            'from' => $from,
            'to' => $to,
        ]);
        throw new \Exception(
